...
namespace Drupal\cp_import\AppImport\os_classes_import;
 ...
 * Class AppImport.
 *
 * @package Drupal\cp_import\AppImport\os_classes_import
 */
class AppImport extends Base {
   ...
   * @var string
   */
  protected $groupPluginId = 'group_node:class';
  ...
  public function preRowSaveActions(MigratePreRowSaveEvent $event) {
    ...
    // Convert semester values to lower.
    $semester_value = $dest_val['field_semester'] ? strtolower($dest_val['field_semester']) : '';
    $row->setDestinationProperty('field_semester', $semester_value);
  }
  ...
  public function validateHeaders(array $data): array {
    ...
    foreach ($data as $row) {
      ...
      foreach ($classHeaders as $classHeader) {
        if (!in_array($classHeader, $columnHeaders)) {
          $missing['@' . $classHeader] = $this->t('<li> @column </li>', ['@column' => $classHeader]);
          $headerMissing = TRUE;
        }
      }
    }
  ...
  public function validateRows(array $data) : array {
    ...
    $options = $this->cpImportHelper->getSemesterFieldValues();
    ...
    foreach ($data as $delta => $row) {
      ...
      if (!empty($row['Semester']) && !in_array(strtolower($row['Semester']), $options)) {
        $semesterRows .= $row_number . ',';
      }
    }
    ...
    $semesterRows = rtrim($semesterRows, ',');
    if ($semesterRows) {
      $msg_arr = $this->getErrorMessage($semesterRows, 'Semester value is invalid.');
      $message['@semester'] = $msg_arr['message'];
      $msg_count += $msg_arr['count'];
      $hasError = TRUE;
    }
