...
namespace Drupal\cp_import\AppImport\os_presentations_import;
 ...
 * Class AppImport.
 *
 * @package Drupal\cp_import\AppImport\os_presentations_import
 */
class AppImport extends Base {
   ...
   * @var string
   */
  protected $groupPluginId = 'group_node:presentation';
  ...
  public function preRowSaveActions(MigratePreRowSaveEvent $event) {
    ...
    if ($originalDate = $source['Date']) {
      $newDate = date("Y-m-d", strtotime($originalDate));
      $row->setDestinationProperty('field_presentation_date', $newDate);
    }
  ...
  public function validateRows(array $data) : array {
    ...
    foreach ($data as $delta => $row) {
      ...
      // Validate Date.
      if ($presentation_date = $row['Date']) {
        if (!strtotime($presentation_date)) {
          $presentationDateRows .= $row_number . ',';
        }
      }
      // Validate File url.
      if ($url = $row['Files']) {
    ...
    $presentationDateRows = rtrim($presentationDateRows, ',');
    if ($presentationDateRows) {
      $msg_arr = $this->getErrorMessage($presentationDateRows, 'Presentation date format is invalid.');
      $message['@presentation_date'] = $msg_arr['message'];
      $msg_count += $msg_arr['count'];
      $hasError = TRUE;
    }
